...
class Input extends React.Component {
  ...
  handlePress = () => {
    const { onPress, disabled, onFocus } = this.props;
  ...
  renderLabel = () => {
    const { header, label, textColor, textProps } = this.props;

    const TextToUse = header ? HText : PText;

    if (label) {
      return <TextToUse text={label} color={textColor} {...textProps} />;
    }
  ...
  renderWordCount = () => {
    const { wordCount, textColor, wordCountBgColor } = this.props;
...
Input.defaultProps = {
  activeOpacity: 0.9,
  placeholderTextColor: getGlobalColors().gray,
